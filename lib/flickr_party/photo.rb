  ...
  class Photo
    ...
    def url(size=nil)
      if %w(m s t b).include?(size)
        "https://farm#{@hash['farm']}.static.flickr.com/#{@hash['server']}/#{@hash['id']}_#{@hash['secret']}_#{size}.jpg"
      elsif size == 'o' and @hash['originalsecret'] and @hash['originalformat']
        "https://farm#{@hash['farm']}.static.flickr.com/#{@hash['server']}/#{@hash['id']}_#{@hash['originalsecret']}_#{size}.#{@hash['originalformat']}"
      elsif size.nil? or size == '-'
        "https://farm#{@hash['farm']}.static.flickr.com/#{@hash['server']}/#{@hash['id']}_#{@hash['secret']}.jpg"
      else
